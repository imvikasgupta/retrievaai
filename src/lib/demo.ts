...
const DEMO_ANSWERS: DemoAnswer[] = [
  {
    ...
    answer:
      "According to Retrieva's **Refund & Cancellation Policy**, eligible customers can request a refund within **14 days** of an initial purchase or a plan upgrade, provided the workspace has not used more than 25% of its monthly message allowance.\n\n- Refunds are returned to the original payment method within 5–10 business days.\n- Subscriptions can be cancelled any time from **Settings → Billing** and remain active until the end of the billing period.",
    sources: [
      {
        ...
        similarity: 0.96,
        content:
          "Refund eligibility: Retrieva offers a full refund within 14 days of an initial purchase or a plan upgrade…",
      },
    ],
  },
  {
...
export function demoAnswer(question: string): { answer: string; sources: RetrievedSource[]; grounded: boolean } {
  ...
  if (!hit) {
    return {
      answer:
        "I couldn't find enough information in the knowledge base to answer this accurately. In Demo Mode I can only answer from the five sample Retrieva documents.",
      sources: [],
      grounded: false,
    };
  }
